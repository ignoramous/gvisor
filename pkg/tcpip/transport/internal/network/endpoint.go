...
func (e *Endpoint) AcquireContextForWrite(opts tcpip.WriteOptions) (WriteContext, tcpip.Error) {
	...
	var toOnTheStack tcpip.FullAddress
	to := opts.To
	info := e.Info()
	switch {
	case to == nil:
		...
		// to.
		toOnTheStack = tcpip.FullAddress{
		...
		if ipv6PktInfoValid {
			...
			if pktInfoNICID != 0 {
				...
				if len(pktInfoAddr) == 0 {
					...
					if len(info.BindAddr) != 0 && e.stack.CheckLocalAddress(pktInfoNICID, header.IPv6ProtocolNumber, info.BindAddr) == 0 {
