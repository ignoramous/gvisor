...
func TestIPv6PacketInfo(t *testing.T) {
	...
	tests := []testCase{
		...
		{
			name:       "BindToDevice & Bind wildcard & SendTo with packet info NIC",
			boundNICID: nicID2,
			bindAddr: tcpip.FullAddress{
				Addr: "",
				Port: port,
		...
		{
			name: "Connect then Send with packet info NIC matching",
			connectAddr: tcpip.FullAddress{
				Addr: ipv6RemoteAddr1,
				Port: port,
			},
		...
		// Connect and SendTo
		{
			name: "Connect with NIC then Sendto with differnt NIC with packet info NIC matching sendto NIC",
			connectAddr: tcpip.FullAddress{
				NIC:  nicID2,
